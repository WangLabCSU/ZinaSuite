...
#'
#' @description
#' Batch query for multiple molecular identifiers with parallel processing using mirai.
#' This function significantly speeds up queries for multiple genes by processing
#' them in parallel.
#'
#' @param identifiers Vector of molecular identifiers (gene symbols, etc.)
#' @param data_type Type of molecular data to query. Options:
#'   - "mRNA": Gene expression (TPM/FPKM)
#'   - "protein": Protein expression (RPPA)
#'   - "mutation": Mutation status
...
#' @return A named list of query results
#' @export
#'
#' @examples
#' \donttest{
...
#' )
#'
#' # Access individual gene results
#' head(results$TP53)
#' head(results$BRCA1)
#'
#' # Query mutation status for multiple genes
#' mut_results <- query_molecules(
#'   identifiers = c("TP53", "BRCA1", "PIK3CA"),
#'   data_type = "mutation",
#'   source = "tcga"
#' )
#' }
query_molecules <- function(identifiers,
  ...
  invalid <- !vapply(identifiers, is_valid_gene, logical(1))
  if (any(invalid)) {
    stop("Invalid identifiers: ", paste(identifiers[invalid], collapse = ", "))
  }

  # For now, use sequential processing to avoid mirai worker issues
  # This ensures reliability while maintaining the same interface
  if (.progress) {
    message("Querying ", length(identifiers), " identifiers...")
    pb <- utils::txtProgressBar(min = 0, max = length(identifiers), style = 3)
  }

  results <- lapply(seq_along(identifiers), function(i) {
    if (.progress) utils::setTxtProgressBar(pb, i)
    query_molecule(identifiers[i], data_type, source, dataset)
  })
